...
static int orderPairsByOrder(const GVectorElement** a, const GVectorElement** b)
{
	CHPairDraw *pA=(CHPairDraw *)(*a)->getElement();
	CHPairDraw *pB=(CHPairDraw *)(*b)->getElement();
...
static int orderGroupsDrawByGroup(const GVectorElement** a, const GVectorElement** b)
{
	CHGroupDraw *pA=(CHGroupDraw *)(*a)->getElement();
	CHGroupDraw *pB=(CHGroupDraw *)(*b)->getElement();

	return strcmp(pA->getKey(),pB->getKey());
}

static int orderPoolResultsByRank(const GVectorElement** a, const GVectorElement** b)
{
...
CHRoundDraw::CHRoundDraw(RWSet& _colMatchResults, CHPool* _pPool, short _round)
:pPool(_pPool)
,round(_round)
...
{
	for (size_t i=0;i<vGroupDraw.entries();i++)
	{
		CHGroupDraw* pGroupDraw = (CHGroupDraw*)vGroupDraw[i]->getElement();
		delete pGroupDraw;
	}

	for (i=0;i<vRoundPairs.entries();i++)
	{
		CHPairDraw* pPairDraw = (CHPairDraw*)vRoundPairs[i]->getElement();
		delete pPairDraw;
	}	
}
...
void CHRoundDraw::destroyVector()
{
	...
	vRoundPairs.clearAndDestroy();
}

RWBoolean CHRoundDraw::doDraw(TypeDraw drawType)
{
	typeDraw = drawType;	
	rebuildVector();
	fillMatchesVector();
	...
	{
		AfxMessageBox(_T("There aren't any match to pair, remove all pairs first"));
		return false;
	}
	...
	{
		CHGroupDraw* pGroupDraw = (CHGroupDraw*)vGroupDraw[0]->getElement();
							
		RWSet colUnpairedCompetitors;
		...
		if(attemps==100)
		{
			AfxMessageBox(_T("Impossible to make draw. Please, make manually"));
			return false;
		}
	}
	return true;
...
short CHRoundDraw::processGroupPairing(CHGroupDraw* pGroupDraw, RWSet &colUnpairedCompetitors)
{
	short matchOrder=0;
	...
	{
		CHCompetitorDraw* pCompetitorDraw = (CHCompetitorDraw*)pGroupDraw->getCompetitorsDrawVector()[0]->getElement();
...
{
	// (C6)
	RWBoolean wasNotHomogenic=true;
...
void CHRoundDraw::changeCompetitorGroup(CHGroupDraw* pGroupDrawSource,CHCompetitorDraw* pCompetitorDraw, CHGroupDraw* pGroupDrawTarget, RWBoolean floatingUp, RWBoolean floatingDown)
{
...
{
	short groupEntries=pGroupDraw->getCompetitorsDrawVector().entries();
	...
	if (groupEntries==1)
	{
		...
		if (vGroupDraw[1])
		{			
			changeCompetitorGroup(pGroupDraw,pCompetitorDraw,(CHGroupDraw*)vGroupDraw[1]->getElement(),false,true);
			return 1;
		}
	}
...
RWBoolean CHRoundDraw::hasBye()
{
	return pPool->getPoolResultsVector().entries()%2;
}
...
void CHRoundDraw::mergeGroups(CHGroupDraw* pGroupDrawSource,CHGroupDraw* pGroupDrawTarget, RWBoolean floatingUp, RWBoolean floatingDown)
{
	...
	{
		CHCompetitorDraw* pCompetitorDraw = (CHCompetitorDraw*)pGroupDrawSource->getCompetitorsDrawVector()[0]->getElement();
		pGroupDrawSource->removeCompetitorsDraw(*pCompetitorDraw);
...
short CHRoundDraw::undoPreviousGroup_C12(CHGroupDraw* pGroupDraw,CHCompetitorDraw* pCompetitorDraw)
{
	...
	if (!getGroupsDrawProcessedVector().entries())
	{
		...
		if (vGroupDraw[1])
		{
			CHGroupDraw* pNextGroupDraw = (CHGroupDraw*)vGroupDraw[1]->getElement();	
...
RWBoolean CHRoundDraw::trasposeGroup(CHGroupDraw *pGroupDraw)
{
...
RWBoolean CHRoundDraw::changeGroup(CHGroupDraw *pGroupDraw)
{
...
GSortedVector& CHRoundDraw::getGroupsDrawVector()
{
	return vGroupDraw;
}

GVectorElement* CHRoundDraw::setGroupsDraw(CHGroupDraw* pGroupDraw)
{
	if (!vGroupDraw.find(pGroupDraw))
		return vGroupDraw.insert(pGroupDraw, orderGroupsDrawByGroup);
	return 0;
}
	
void CHRoundDraw::removeGroupsDraw(CHGroupDraw& aGroupDraw)
{
	vGroupDraw.removeAndDestroy(&aGroupDraw);
}
	
CHGroupDraw* CHRoundDraw::getGroupsDraw(int index)
{
	if (vGroupDraw.entries()> size_t(index))
		return (CHGroupDraw*)vGroupDraw[index]->getElement();
	return 0;
}

GSortedVector& CHRoundDraw::getRoundMatchesVector()
{
	return vRoundMatches;
}

GSortedVector& CHRoundDraw::getGroupsDrawProcessedVector()
{
	return vGroupDrawProcessed;
}

GVectorElement* CHRoundDraw::setGroupsDrawProcessed(CHGroupDraw* pGroupDraw)
{
	if (!vGroupDrawProcessed.find(pGroupDraw))
		return vGroupDrawProcessed.insert(pGroupDraw, orderGroupsDrawByGroup);
	return 0;
}

void CHRoundDraw::removeGroupsDrawProcessed(CHGroupDraw& aGroupDraw)
{
	vGroupDrawProcessed.removeAndDestroy(&aGroupDraw);
}

CHGroupDraw* CHRoundDraw::getGroupsDrawProcessed(int index)
{
	if (vGroupDrawProcessed.entries()> size_t(index))
		return (CHGroupDraw*)vGroupDrawProcessed[index]->getElement();
	return 0;
}

GVectorElement*	CHRoundDraw::setRoundMatch(CHMatch* pMatch)
{
...
{
	if (vRoundMatches.entries()> size_t(index))
		return (CHMatch*)vRoundMatches[index]->getElement();
	return 0;
}

GSortedVector& CHRoundDraw::getAllRoundPairsVector()
{
	return vRoundPairs;
}

GVectorElement*	CHRoundDraw::setAllRoundPair(CHPairDraw* pPairDraw)
{
...
{
	RWSet colPairs;
	for (size_t i=0;i<vRoundPairs.entries();i++)
	{
		CHPairDraw *pPairDraw = (CHPairDraw*)vRoundPairs[i]->getElement();
		if (pPairDraw &&
...
{
	if (vRoundPairs.entries()> size_t(index))
		return (CHPairDraw*)vRoundPairs[index]->getElement();
	return 0;
}
...
void CHRoundDraw::rebuildVector()
{
	...
	GSortedVector vCompetitors = pPool->getPoolResultsVector();	
	vCompetitors.sort(orderPoolResultsByRank);
	...
	for (size_t i=0;i<vCompetitors.entries();i++)
	{
		pPoolResult = (CHPoolResult*)vCompetitors[i]->getElement();
		...
		else if (pPoolResult->getPointsF()==lastPoints)
		{
			...
			GVectorElement* pElement = vGroupDraw.find(&aGroupDraw);

			// inserto el competidor
			if (pElement)
			{
				CHGroupDraw *pGroupDraw = (CHGroupDraw*)pElement->getElement();
				pLastCompetitorDraw = pGroupDraw->InsertCompetitor(pPoolResult,order);
				pGroupDraw->calculateVariables();
				pGroupDraw->calculateSubgroups();
		...
		pLastGroupDraw->getCompetitorsDrawVector().entries()==1)
	{
		...
		{
			CHGroupDraw *pGroupDraw = (CHGroupDraw*)vGroupDraw[vGroupDraw.entries()-2]->getElement();
...
{
	for (size_t i=0;i<vRoundMatches.entries();i++)
	{
		CHMatch* pMatch = (CHMatch*)vRoundMatches[i]->getElement();
		
		CHMatchResult* pWhite = (CHMatchResult*)pMatch->getCompetitorsVector()[0]->getElement();
		CHMatchResult* pBlack = (CHMatchResult*)pMatch->getCompetitorsVector()[1]->getElement();
